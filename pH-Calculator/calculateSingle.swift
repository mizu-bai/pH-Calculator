...
func calculateSingle(acidBase: Int, polyprotic: Int, KValue: [Double], cValue: Double) -> Double {
    ...
    if(polyprotic == 1) {
        ...
        cxValue = solveEquation(maxIndex: 3, coefficient: [1, KValue[0], (-KValue[0] * cValue + KwValue), -(KValue[0] * KwValue)], initialValue: cValue)
        ...
        cxValue = solveEquation(maxIndex: 5, coefficient: [1, KValue[0], (KValue[0] * KValue[1] - KValue[0] * cValue - KwValue), (KValue[0] * KValue[1] * KValue[2] - 2 * KValue[0] * KValue[1] - KValue[0] * KwValue), -(3 * KValue[0] * KValue[1] * KValue[2] + KValue[0] * KValue[2] * KwValue), -(KValue[0] * KValue[1] * KValue[2])], initialValue: cValue)
